...
class NPCModel(SingleModelBase):
    ...
    def change_cash(self, amount, trigger_uid, is_increase=True):
        ...
        if is_increase:
            ...
            if self.cash < amount:
                return False
            self.cash -= amount
        self.app.send(f"Player-{self.map}", {"code": 200, "uri": "changeCash", "uid": trigger_uid,
                                             "data": {"uid": self.get_token(), "cash": self.cash, "amount": amount,
    ...
    def add_event(self, event):
        ...
        self.app.send(f"Player-{self.map}", {"code": 200, "uri": "finishAction", "uid": self.get_token(),
                                             "data": {"uid": self.get_token(), "action": event["action"],
                                                      "startTime": event["time"], "endTime": self.app.last_game_time}})
        events.append(event)
        events = events[::-1]
        if len(events) > 3:
            events = events[:3]
    ...
    def set_action(self, action):
        ...
        for key, value in action.items():
            push_info[key] = value
        self.app.send(f"Player-{self.map}",
                      {"code": 200, "uri": "newAction", "uid": self.get_token(), "data": push_info})
        self.act = action
    ...
    def add_chat(self, uid, content, isSender=True):
        ...
        if chats and self.app.last_game_time - chats[-1]["createTime"] >= 1000 * 60 * 60 * 3:
            chats = list()
        assert type(
            content) is str and ': ' in content, f' content should be a string with ": ", your content is {content} '
        chats.append(
            {"createTime": self.app.last_game_time, "content": content.partition(": ")[2], "isSender": isSender})
        chats = chats[::-1]
        if len(chats) > 10:
            chats = chats[:10]
        self.chats[uid] = chats
        if not isSender:
            self.new_chats.append(
                {"sender": uid, "createTime": self.app.last_game_time, "content": content.partition(": ")[2]})
