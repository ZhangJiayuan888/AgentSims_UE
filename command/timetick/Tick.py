...
class Tick(CommandBase):
    ...
    def next_time(self) -> bool:
        ...
        next_day = False
        if self.app.get_formatter(new_game_time, "%Y-%m-%d") > self.app.get_formatter(self.app.last_game_time,
                                                                                      "%Y-%m-%d"):
            next_day = True
    ...
    async def move(self, entity_model, map_id, map_model, entity_type, uid) -> int:
        ...
        moves = len(path)
        if moves:
            self.app.send(f"Player-{map_id}", {"code": 200, "uri": "movePath", "uid": uid, "data": {"uid": uid,
                                                                                                    "path": [{"x": x[0],
                                                                                                              "y": x[1]}
                                                                                                             for x in
                                                                                                             path[
            ...
            # Fix: Protect against map_model errors (NoneType subscriptable)
            try:
                sight = map_model.search_sight(entity_model.x, entity_model.y)
            except Exception as e:
                print(f"[Warn] search_sight failed for {uid}: {e}")
                sight = {"people": [], "equipments": []}
    ...
    async def _execute_chat(self, map_id, map_model, uid, entity_model, tuid, target_model, chats, topic=""):
        ...
        try:
            ...
            target_sight = {"people": [], "equipments": []}

        entity_model.add_event(entity_model.act)
        entity_model.set_action(
            {"action": f"Talking to {target}", "time": self.app.last_game_time, "target": target, "targetID": tuid})
        while len(chats) < 10:
            ...
            if ": " not in content:
                content = f"{target_model_loop.name}: {content}"
        ...
        self.app.send(f"Player-{map_id}", {"code": 200, "uri": "chatWith", "uid": uid, "data": {"chats": [
            {"content": chat["content"].partition(": ")[2], "speaker": chat["speaker"],
             "speakerID": tuid if target == chat["speaker"] else uid} for chat in chats]}})
    ...
    async def parse_react(self, result: Dict[str, Any], entity_model, map_id, map_model, uid):
        ...
        if "newPlan" in result["data"]:
            try:
                ...
                # Fix: Removed remote_pdb
                print(f"[Warn] No purpose in newPlan for {uid}")
                entity_model.plan = "Idle"

            self.app.send(f"Player-{map_id}",
                          {"code": 200, "uri": "newPlan", "uid": uid, "data": {"uid": uid, "plan": entity_model.plan}})
            entity_model.save()
            ...
            if location != curLocation:
                ...
                if buildings_model:
                    ...
                    if building:
                        ...
                        targetX, targetY = map_model.get_empty_tile(building['lx'], building['ty'], building['rx'],
                                                                    building['by'])
                        ...
                        if path:
                            ...
                            self.app.send(f"Player-{map_id}", {"code": 200, "uri": "planToMove", "uid": uid,
                                                               "data": {"uid": uid, "targetBuilding": location,
            ...
            if tuid and tuid.partition("-")[0] == "NPC":
                ...
                content = result["data"].get('chat', {}).get('content', '...')

                # Fix: Ensure content format "Speaker: Message"
                if not isinstance(content, str): content = str(content)
                if ": " not in content:
                    content = f"{entity_model.name}: {content}"

                chats = [{"speaker": entity_model.name, "content": content}]
            ...
            if equipments_model:
                equipment_id, equipment_model = equipments_model.get_equipment_by_name_and_position(equipment,
                                                                                                    entity_model.x,
                                                                                                    entity_model.y)
                if equipment_model:
                    lx, ty, rx, by = equipment_model["lx"], equipment_model["ty"], equipment_model["rx"], \
                    equipment_model["by"]
            ...
            # Fix: Cap continue_time to prevent agents from getting stuck for too long
            raw_time = result['data']['use'].get('continue_time', 10)
            try:
                continue_time = min(int(raw_time), 10)
            except:
                continue_time = 10
            ...
            entity_model.add_event(entity_model.act)
            entity_model.set_action(
                {"action": f"{operation}", "time": self.app.last_game_time, "equipment": equipment_id,
                 "continue_time": continue_time, "cost": cost, "earn": earn})
    ...
    async def finish_using(self, entity_model, map_id, map_model, uid) -> int:
        ...
        try:
            ...
            print(f"[Warn] search_sight error in finish_using for {uid}: {e}")
            sight = {"people": [], "equipments": []}
        ...
        if result["status"] == 500:
            ...
            # Fix: Handle NoneType for result['data']
            if result.get('data') is None:
                # removed remote_pdb
                print(f"[Warn] Result data None for {uid} in finish_using. Initializing empty dict.")
                result['data'] = {}
    ...
    async def solve_init(self, init):
        # Fix: Check type of init to avoid TypeError
        uid = ""
        ...
        counter = 0
        entity_type, entity_id, entity_model, map_id, map_model = self.get_entity_model(uid)
        if not entity_model:
            return counter
        if not map_model:
            return counter
...
#             except:
#                 __import__('remote_pdb').set_trace()
#             self.app.send(f"Player-{map_id}", {"code": 200, "uri": "newPlan", "uid": uid,
#                                                "data": {"uid": uid, "plan": result["data"]["newPlan"]["purpose"]}})
#             entity_model.save()
#             location = result['prompts']['plan']['building']
#             buildings_model = self.get_single_model("Buildings", map_id, create=False)
#             curLocation = map_model.map.get(str(entity_model.x), dict()).get(str(entity_model.y), dict()).get(
